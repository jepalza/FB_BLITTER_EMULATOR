...
Function m6809_execute() As Integer
  ...
  Static DIR_BREAK As Integer=-1 '&hf67b ' direccion donde queremos parar (-1 anula)
  Static debug As Integer=0 ' si es 1, se para nada mas iniciarse
  Static DirIni As Integer=&hc800 ' zona de la RAM a visualizar
...
'GoTo nodebug
  ...
  For FF=0+dirini To 1871+dirini -(52*15) ' con la resta, quitamos altura de pantalla, para que vaya mas rapido
  	Locate AA,BB
  	 ...
  	 If CC>255 Then SS=Chr(254) ' cuadro blanco
  	 If SS=" " Then SS=Chr(249) ' punto blanco (en el medio)
  	 Print SS;
...
'End If
  ...
  'If debug=1 Then refresco=-1:Sleep
...
Sub pantalla
        ...
        Dim Escala As Integer=2
        Dim Ancho As Integer=304
        Dim Alto As Integer=256
        Dim IniScreen As Integer=0
        Dim xx As Integer
        Dim yy As Integer
        Dim aa As Integer
        Dim cc As Integer
        ...
        Dim ff As integer
		  ...
		  XX=0:YY=IniScreen
		  AA=((Ancho/8)*alto)*4 ' 304x256x4 (16 colores=4 planos)
		  For FF=0 To AA-1
		  	  CC=VRAM(FF) Shr 4
		  	  DD=(VRAM(FF) And &H0F)
		      Line (XX*escala,YY*escala)-Step(escala+1,escala+1),CC,bf
		      Line ((XX+1)*escala,YY*escala)-Step(escala+1,escala+1),DD,bf
		    YY+=1
		    If YY>(alto+IniScreen-1) Then YY=IniScreen:XX+=2
		  Next
